...
namespace _004_Yield
{
    ...
    //Т.е оператор yield занимается тем, что генерит специальный класс, который выступает в роли итератора(перечислителя) для коллекции
    //(там внутри хардкодятся switch`ами и cas`ами все элементы коллекции)
