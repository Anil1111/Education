...
namespace _002_ICollection_Generic
{
    class UserCollection<T> : ICollection<T>
    {
        ...
        // bool IsReadOnly { get; } // возвращает bool, является ли коллекция IsReadOnly (нельзя ничего записать, а можно только читать)

        // void Add(T item); //Повзоляет добавлять элемент в коллекцию
